...
- `list-sessions` はソケットの有無で生死判定するため、server が異常終了してソケット残骸が残ると EXITED が付かず「生きて見える」ゾンビになる → `Session.Zombie` で検出（実行中 server プロセスのコマンドライン走査で `--server …\<name>` 不在を判定）。掃除は `PurgeSocket`（`os.Remove`）→ `KillCmd`（`delete-session -f`）。UI は `(応答なし)` 減光表示＋アタッチ無効化。
